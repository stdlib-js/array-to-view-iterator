...
function arrayview2iterator( src ) {
	...
	var i;
	if ( !isCollection( src ) ) {
		throw new TypeError( format( '01l2O,GW', src ) );
	}
	...
	if ( nargs === 1 ) {
		...
		fcn = arguments[ 3 ];
		if ( !isFunction( fcn ) ) {
			throw new TypeError( format( '01l32,MM', fcn ) );
		}
		thisArg = arguments[ 4 ];
	}
	if ( !isInteger( begin ) ) {
		throw new TypeError( format( '01lMN', begin ) );
	}
	if ( !isInteger( end ) ) {
		throw new TypeError( format( '01lMO', end ) );
	}
	if ( end < 0 ) {
		end = src.length + end;
		if ( end < 0 ) {
